...
System::Void com_port_monitor::MainForm::MainForm_Load(System::Object^ sender, System::EventArgs^ e)
{
	int bt{};
	bool ia{};
	readJsonSetting(bt, ia);
	this->baudRate = bt;
	this->checkBoxAutoCon->Checked = this->isAutoExtern = ia;
...
bool com_port_monitor::MainForm::connectToPort()
{
	...
	if (this->checkBoxAutoCon->Enabled)
	{
		this->comboBoxComPorts->Items->Clear();
		this->chooseComToolStripMenuItem->DropDownItems->Clear();
		this->comboBoxComPorts->ResetText();
		for (int i = 1; i <= 256; i++)
		{
			if (check_com_port(i) != -1)
			{
				...
				this->chooseComToolStripMenuItem->DropDownItems->Add(p);
				this->comPort = "COM" + i;
				this->comboBoxComPorts->Text = this->comPort;
			}
		}
	}
...
void com_port_monitor::MainForm::setBaudRate(System::Object^ sender, System::EventArgs^ e)
{
	...
	setJsonSetting(this->baudRate, this->isAutoExtern);
}
...
System::Void com_port_monitor::MainForm::checkBoxAutoCon_CheckedChanged(System::Object^ sender, System::EventArgs^ e)
{
	this->isAutoExtern = this->checkBoxAutoCon->Checked;
	setJsonSetting(this->baudRate, this->isAutoExtern);
}
